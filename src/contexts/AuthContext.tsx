...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const signIn = useCallback(async ({ email, password }: SignInCredentials) => {
    ...
    // O hub é só para membros da organização. Médico credenciado tem
    // linha em userEnterprise, mas o endpoint só devolve onde ele é
    // membro — então cai aqui e não entra.
    ...
    if (enterprises.length === 0) {
      throw new Error(
        'Esta conta não tem acesso ao portal de gestão. Se você é médico credenciado, use o aplicativo Cobreai. Se você deveria gerenciar a organização, peça ao administrador para adicioná-lo em Membros.',
      );
    }
